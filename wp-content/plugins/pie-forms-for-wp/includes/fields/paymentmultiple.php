...
class PFORM_Fields_PaymentMultiple extends PFORM_Abstracts_Fields {
	...
	public function field_preview( $field , $form_data ) {
		...
		do_action('pie_forms_payment_multiple_field_builder_insider' , $field , $form_data, $this);
	...
	public function field_display( $field, $field_atts, $form_data ) {
		...
		do_action('pie_forms_payment_multiple_field_frontend' , $field, $field_atts, $form_data);
	...
	function pie_forms_display_label_before_print($to_show, $field){
		...
		if(isset($field['type']) && $field['type'] == 'payment-multiple' && (!Pie_Forms()->core()->pform_check_addon_exist('pie-forms-for-wp-paypal-payment/pie-forms-for-wp-paypal-payment.php') || $is_addon_activated == 'Deactivated') && (!Pie_Forms()->core()->pform_check_addon_exist('pie-forms-for-wp-stripe/pie-forms-for-wp-stripe.php') || $is_addon_activated_stripe == 'Deactivated') && (!Pie_Forms()->core()->pform_check_addon_exist('pie-forms-for-wp-authorize-net/pie-forms-for-wp-authorize-net.php') || $is_addon_authorize_net == 'Deactivated')){
			$to_show = true;
		}
